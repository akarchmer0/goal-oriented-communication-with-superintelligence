...
from .env import VectorizedAlanineEnv
...
def collect_trajectory(
    ...
    visible_gradient_env: VectorizedAlanineEnv | None = None,
...
def maybe_save_trajectory_plot(
    ...
    energy_range = max(float(env.max_objective_env[0]), 1e-8)
    grid_energy_normalized = np.clip(
        (grid_energy - env.global_min_energy) / energy_range, 0.0, 1.0
    ).astype(np.float32)
...
def evaluate_optimization_curves(
    *,
    ...
    hidden_gradient_env: VectorizedAlanineEnv,
    no_oracle_model: PolicyValueNet | None,
    no_oracle_env: VectorizedAlanineEnv | None,
    visible_gradient_model: PolicyValueNet | None,
    visible_gradient_env: VectorizedAlanineEnv | None,
) -> dict | None:
    ...
    plot_spatial_optimization_curves_by_method(
        ...
        title=f"Alanine optimization curves per method (seed={config.seed}, tasks={num_tasks})",
    ...
    plot_spatial_optimization_curve_summary(
        ...
        title=f"Alanine optimization mean +/- std (seed={config.seed}, tasks={num_tasks})",
        method_labels=OPTIMIZATION_METHOD_LABELS,
    )
...
def _synchronize_envs(
    reference_env: VectorizedAlanineEnv,
    target_env: VectorizedAlanineEnv,
) -> None:
...
# ---------------------------------------------------------------------------
# Helper: build env with SDP
# ---------------------------------------------------------------------------

def _build_env_with_sdp(
    config: TrainConfig,
    oracle: SpatialOracle,
    seed: int,
) -> tuple[VectorizedAlanineEnv, "LiftingMap", object, np.ndarray, float, str]:
    """Load energy, build lifting map, solve SDP, construct env."""
    ...
    return env, lifting_map, energy_surface, s_star_sdp, sdp_bound, sdp_status
...
def run_training(config: TrainConfig, return_artifacts: bool = False) -> dict:
    ...
    print(f"Loading energy surface...")
    energy_surface = load_energy_surface(
        config.energy_json,
        use_synthetic=config.use_synthetic_fallback,
        d=config.synthetic_d,
    ...
    print(f"  d={energy_surface.d}, K_energy={energy_surface.K_energy}, {len(energy_surface.minima)} known minima")
    ...
    with (run_dir / "sdp_info.json").open("w", encoding="utf-8") as handle:
        json.dump(sdp_info, handle, indent=2)
    ...
    print(f"Constructing environment with SDP oracle...")
    env = VectorizedAlanineEnv(
        lifting_map=lifting_map,
        energy_surface=energy_surface,
        s_star_sdp=s_star_sdp,
    ...
    no_oracle_env: VectorizedAlanineEnv | None = None
    no_oracle_model: PolicyValueNet | None = None
    ...
    if config.enable_baselines and config.oracle_mode != "no_oracle":
        ...
        no_oracle_env = VectorizedAlanineEnv(
        ...
        if config.oracle_mode == "convex_gradient":
            ...
            visible_gradient_env = VectorizedAlanineEnv(
                lifting_map=lifting_map, energy_surface=energy_surface, s_star_sdp=s_star_sdp,
    ...
    if config.enable_training_plots:
        ...
        maybe_save_trajectory_plot(
            ...
            title=f"Alanine dipeptide | Kmap={config.K_map}, {config.oracle_mode}",
        )
...
def parse_args() -> tuple[TrainConfig, list[int]]:
    ...
    parser = argparse.ArgumentParser(description="Alanine dipeptide RL training with SDP oracle")
...
def main() -> None:
    ...
    summary_stem = base_run_name or f"alanine_{config.oracle_mode}_{config.sensing}_multi_seed"
