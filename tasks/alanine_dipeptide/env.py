"""Vectorized RL environment for alanine dipeptide torus optimization with SDP oracle.

Mirrors the interface of VectorizedTripeptideEnv but operates on the 2D torus
[0, 2pi)^2 with d=2 backbone torsion angles (phi, psi).
...
class VectorizedAlanineEnv:
    ...
    # ------------------------------------------------------------------
    # Energy landscape grid (for Ramachandran plotting)
    # ------------------------------------------------------------------

    def energy_landscape_grid(
        self, resolution: int = 140, env_index: int = 0
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Return energy on a (phi, psi) grid over [0, 2pi)^2."""
