...
@dataclass
class TrainConfig:
    task: str = "alanine_dipeptide"
    K_map: int = 6
    K_relax: int = 4
    energy_json: str = "tasks/alanine_dipeptide/fourier_coefficients.json"
    use_synthetic_fallback: bool = False
    synthetic_d: int = 2
    synthetic_K: int = 6
    synthetic_n_minima: int = 6
    visible_dim: int = 2
    ...
    def spatial_hidden_dim(self) -> int:
        """Hidden dim is determined by the lifting map dimension D = 2*N_freq."""
        N_freq = enumerate_frequency_indices(self.visible_dim, self.K_map).shape[0]
        return 2 * N_freq
    ...
    def resolve_run_dir(self) -> Path:
        ...
        default_run_name = (
            f"alanine_Kmap{self.K_map}_Krelax{self.K_relax}_D{self.spatial_hidden_dim}_"
