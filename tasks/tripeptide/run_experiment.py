"""Multi-seed experiment runner for tripeptide optimization.

Compares hidden-gradient PPO, visible-gradient PPO, no-oracle PPO, Adam, GD,
and random search on the 4D torus energy surface.

Example usage:
    python -m tasks.tripeptide.run_experiment \
        --seeds 0,1,2 --train_steps 500000 --K_map 2
"""
...
def _plot_objective_vs_step(*, aggregate: dict[str, Any], output_path: Path) -> None:
    ...
    ax.set_title(
        "Tripeptide | normalized objective vs optimization step",
        loc="left",
        fontsize=11,
        pad=10,
    )
    ax.set_xlabel("Optimization step")
    ax.set_ylabel("Normalized energy E(z)")
...
def _plot_final_objective_summary(*, aggregate: dict[str, Any], output_path: Path) -> None:
    ...
    ax.set_title(
        "Tripeptide | final objective (mean across seeds with 95% CI)",
        loc="left",
        fontsize=11,
        pad=10,
    )
...
def _plot_success_rate_vs_step(*, aggregate: dict[str, Any], output_path: Path) -> None:
    ...
    ax.set_title(
        "Tripeptide | cumulative success rate vs optimization step",
        loc="left",
        fontsize=11,
        pad=10,
    )
...
def _plot_final_success_rate_summary(*, aggregate: dict[str, Any], output_path: Path) -> None:
    ...
    ax.set_xlabel("Method")
    ax.set_title(
        "Tripeptide | success rate (objective <= 0.01)",
        loc="left",
        fontsize=11,
        pad=10,
    )
    ...
    *,
    ...
    for task_index in range(num_plot_tasks):
        output_path = seed_plots_root / (
            f"tripeptide_trajectory_task{int(task_index + 1):02d}.png"
        )
        maybe_save_trajectory_plot(
            model=hidden_model,
            env=hidden_env,
            ...
            title=(
                f"Tripeptide trajectory | seed={int(seed)}, "
...
def _run_study(
    *,
    ...
    for seed in seeds:
        run_name = f"tripeptide_Kmap{int(args.K_map)}_seed{int(seed)}"
        config = _build_train_config(args=args, seed=int(seed), run_name=run_name)
        print(
            f"[tripeptide_experiment] seed={int(seed)} training run={run_name} "
        ...
        if hidden_env is None:
            raise RuntimeError("Expected tripeptide env artifact in training output")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run tripeptide multi-seed optimizer comparison experiment."
    )
    parser.add_argument(
        "--seeds",
        type=str,
    ...
    parser.add_argument(
        "--suite_output_dir",
        type=str,
        default="plots/tripeptide_experiment",
    )
