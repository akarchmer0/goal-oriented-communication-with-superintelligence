...
from .config import RequestedExperimentsConfig, TrainConfig
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Use plot_experiments to generate/re-generate plots."
        )
    )
    parser.add_argument(
        "--n",
        ...
        json.dumps(
            {
                ...
                "seeds_used": seeds,
                "plot_cmd": (
                    f"python3 -m tasks.graph.plot_experiments --manifest "
                    f"{manifest_path}"
                ),
            },
            indent=2,
