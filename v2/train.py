...
def evaluate_spatial_optimization_curves(
    *,
    ...
    if config.task != "spatial" or not config.spatial_enable_optimization_curve_eval:
        return None
...
def maybe_save_spatial_trajectory_plot(
    ...
    plot_spatial_trajectory_with_gradients(
        ...
        title=f"{title} | PPO hidden + GD/Adam",
        landscape_x=grid_x,
        landscape_y=grid_y,
...
def run_training(config: TrainConfig, return_artifacts: bool = False) -> dict:
    ...
    if config.task == "spatial" and config.oracle_mode == "visible_gradient":
        raise ValueError(
    ...
    if config.task == "spatial" and (
        not np.isfinite(float(config.ppo_step_scale)) or float(config.ppo_step_scale) <= 0.0
    ):
        raise ValueError("ppo_step_scale must be finite and > 0")
    if config.task == "spatial" and config.spatial_enable_success_curriculum:
    ...
    spatial_curriculum_enabled = bool(
        config.task == "spatial" and config.spatial_enable_success_curriculum
    )
    ...
    with metric_csv_path.open("w", newline="", encoding="utf-8") as csv_file, metric_jsonl_path.open(
        ...
        if (
            ...
            and config.spatial_fixed_start_target
        ):
            ...
            for _ in range(config.rollout_len):
                ...
                if (
                    ...
                    for vg_env_index in vg_done_indices:
                        ...
                        if isinstance(visible_gradient_env, VectorizedSpatialEnv):
                ...
                for env_index in done_indices:
                    ...
                    if (
                        config.task == "spatial"
                        and isinstance(env, VectorizedSpatialEnv)
                        and env.compute_episode_baselines
                    ):
                    ...
                    if config.eval_interval_episodes > 0 and total_episodes % config.eval_interval_episodes == 0:
                        ...
                        if config.task == "graph":
                            ...
                            tqdm.write(
                                ...
                                f"avg_dist_ref_adam_baseline={float(row['avg_adam_baseline_final_ref_distance']):.4f} "
                                f"avg_dist_ref_no_oracle={float(row['avg_no_oracle_final_ref_distance']):.4f} "
                                f"avg_dist_ref_visible_gradient={float(row['avg_visible_gradient_final_ref_distance']):.4f} "
                                f"avg_path_len={avg_path_len_all:.3f} "
                                f"avg_shortest_dist={avg_shortest_dist_all:.3f}"
                            )
                    if (
    ...
    if config.task == "spatial" and avg_adam_baseline_final_ref_distance_all is not None:
        print(
    ...
    if config.task == "spatial" and avg_visible_gradient_final_ref_distance_all is not None:
        print(
    ...
    summary = {
        ...
        "spatial_enable_baselines": (
            bool(config.spatial_enable_baselines) if config.task == "spatial" else None
        ),
        "spatial_early_stop_on_all_methods_success": (
            bool(config.spatial_early_stop_on_all_methods_success) if config.task == "spatial" else None
        ),
        ...
        "ppo_step_scale": (
            float(config.ppo_step_scale) if config.task == "spatial" else None
        ),
        "spatial_control_budget_scale": (
            float(env.control_budget_scale)
            if (config.task == "spatial" and isinstance(env, VectorizedSpatialEnv))
            else None
        ),
        "spatial_base_step_size": (
            float(env.base_step_size)
            if (config.task == "spatial" and isinstance(env, VectorizedSpatialEnv))
            else None
        ),
        "spatial_effective_step_size": (
            float(env.step_size)
            if (config.task == "spatial" and isinstance(env, VectorizedSpatialEnv))
            else None
        ),
        "spatial_enable_success_curriculum": (
            bool(config.spatial_enable_success_curriculum) if config.task == "spatial" else None
        ),
        "spatial_success_curriculum_start": (
            float(config.spatial_success_curriculum_start) if config.task == "spatial" else None
        ),
        "spatial_success_curriculum_trigger_rate": (
            float(config.spatial_success_curriculum_trigger_rate) if config.task == "spatial" else None
        ),
        "spatial_success_curriculum_decay": (
            float(config.spatial_success_curriculum_decay) if config.task == "spatial" else None
        ),
        "spatial_success_curriculum_min": (
            float(config.spatial_success_curriculum_min) if config.task == "spatial" else None
        ),
        "spatial_final_success_threshold": (
            float(env.success_threshold)
            if (config.task == "spatial" and isinstance(env, VectorizedSpatialEnv))
            else None
        ),
        "spatial_success_curriculum_updates": (
            int(curriculum_updates) if config.task == "spatial" else None
        ),
        "spatial_baseline_lr_gd": (
            float(env.baseline_lr_gd) if (config.task == "spatial" and isinstance(env, VectorizedSpatialEnv)) else None
        ),
        "spatial_baseline_lr_adam": (
            float(env.baseline_lr_adam)
            if (config.task == "spatial" and isinstance(env, VectorizedSpatialEnv))
            else None
        ),
        "spatial_baseline_lr_tuning_enabled": (
            bool(config.spatial_tune_baseline_lrs) if config.task == "spatial" else None
        ),
        "spatial_baseline_lr_tune_tasks": (
            int(config.spatial_baseline_lr_tune_tasks) if config.task == "spatial" else None
        ),
        "spatial_baseline_lr_candidates": (
            str(config.spatial_baseline_lr_candidates) if config.task == "spatial" else None
        ),
...
def parse_args() -> tuple[TrainConfig, list[int]]:
    ...
    parser.add_argument(
        "--oracle_mode",
        type=str,
        choices=sorted((ORACLE_MODES | SPATIAL_ORACLE_MODES) - {"visible_gradient"}),
...
def main() -> None:
    ...
    if config.task == "spatial" and per_seed_spatial_curves:
